"""
Cron Coolify (cada 20 min): relateddetailid live (OC recientes).
...
def main() -> int:
    ...
    try:
        ...
        print(f"[live_sync_related] ERROR: {e}", file=sys.stderr, flush=True)
        return 1
    except Exception as e:
        logging.getLogger(__name__).exception("live_sync_related")
        print(f"[live_sync_related] ERROR: {e}", file=sys.stderr, flush=True)
        return 1

    _print_summary("LIVE SYNC RELATED — SUMMARY", stats)
