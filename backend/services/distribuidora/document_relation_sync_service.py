...
def discover_oc_missing_related_with_probable_sql() -> str:
    """Cola de investigación: OC sin related 1/6 + probable >= 75. No promociona."""
    return """
    WITH ocs AS (
    ...
    direct AS (
        ...
        INNER JOIN distribuidora.documents inv
           ...
           AND inv.document_type_id IN (1, 6)
        INNER JOIN ocs ON ocs.document_id = dd.document_id
    )
    SELECT
