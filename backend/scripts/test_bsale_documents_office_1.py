...
Requisitos:
  ...
  - Pegar token abajo o exportar BSALE_TOKEN en el entorno (si el placeholder sigue igual).
...
Ejecución (desde la raíz del repositorio):
  ...
  2) Exportar conexión y token (o pegar el token en BSALE_TOKEN arriba):
       set PG_HOST=...
       set PG_DB=...
       set PG_USER=...
       set PG_PASSWORD=...
  ...
  3) Ajustar en este archivo DATE_FROM, DATE_TO y BSALE_TOKEN si aplica.

  4) Ejecutar:
       python backend/scripts/test_bsale_documents_office_1.py
"""
...
def _token() -> str:
    t = (BSALE_TOKEN or "").strip()
    if t and t != "PEGAR_TOKEN_AQUI":
        return t
    env_t = (os.getenv("BSALE_TOKEN") or os.getenv("BSALE_TOKEN_SPA") or "").strip()
    if env_t:
        return env_t
    print(
        "[test_bsale_documents] ERROR: defina BSALE_TOKEN arriba en el script "
        "o exporte BSALE_TOKEN / BSALE_TOKEN_SPA en el entorno.",
        file=sys.stderr,
    )
    sys.exit(1)
